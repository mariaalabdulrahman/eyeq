...
const Index = () => {
    ...
    const [viewMode, setViewMode] = useState('textual');
    const [uploadModalOpen, setUploadModalOpen] = useState(false);
    const [sidebarWidth, setSidebarWidth] = useState(320);
    const [chatHeight, setChatHeight] = useState(60); // percentage
    const { scans, activeScan, activeTabId, chatHistory, setActiveTabId, addScan, removeScan, addChatMessage, } = useScanAnalysis();
    const handleUpload = (file, type) => {
        addScan(file, type);
    };
    const handleSendMessage = (message, selectedScanIds) => {
        addChatMessage(message, selectedScanIds);
    };
    return (<div style={{ height: '100vh', display: 'flex', flexDirection: 'column', backgroundColor: '#f8fafc' }}>
      {/* Header */}
      <header style={{
      ...
      {/* Tabs */}
      <ImageTabs scans={scans} activeTab={activeTabId} onTabChange={setActiveTabId} onAddNew={() => setUploadModalOpen(true)} onRemoveTab={removeScan}/>
      ...
      <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
        ...
        <aside style={{
          ...
          <div style={{ height: `${chatHeight}%`, overflow: 'hidden', borderBottom: '1px solid #e5e7eb' }}>
            <ChatSidebar scans={scans} chatHistory={chatHistory} onSendMessage={handleSendMessage}/>
          </div>
          ...
          <div style={{ flex: 1, overflow: 'auto' }}>
            <ViewModeButtons activeMode={viewMode} onModeChange={setViewMode}/>
          </div>
          ...
          <div style={{
            ...
            document.addEventListener('mousemove', onMouseMove);
            document.addEventListener('mouseup', onMouseUp);
        }} onMouseEnter={(e) => e.currentTarget.style.backgroundColor = '#0891b2'} onMouseLeave={(e) => e.currentTarget.style.backgroundColor = 'transparent'}/>
        </aside>
        
        {/* Analysis Panel */}
        <main style={{ flex: 1, overflow: 'hidden' }}>
          <AnalysisPanel scan={activeScan} viewMode={viewMode} allScans={scans} onUploadClick={() => setUploadModalOpen(true)}/>
        </main>
      </div>
