...
export function VisualizationView({ scan }) {
    ...
    return (<div className="h-full flex flex-col gap-4">
      ...
      <div className="flex flex-wrap gap-2 bg-card border border-border rounded-xl p-4">
        {filters.map(({ type, icon: Icon, label }) => (<button key={type} onClick={() => setActiveFilter(type)} className={cn("flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200", activeFilter === type
      ...
      <div className="flex-1 bg-card border border-border rounded-xl p-4 flex flex-col">
        ...
        <div className="flex-1 flex items-center justify-center bg-secondary/30 rounded-lg overflow-hidden relative">
          <img src={scan.imageUrl} alt={scan.name} className="max-w-full max-h-full object-contain transition-all duration-300" style={getFilterStyle()}/>
          
          {/* Overlay for segmentation */}
          {activeFilter === 'segmentation' && (<div className="absolute inset-0 pointer-events-none">
          ...
          {/* Thermal color bar */}
          {activeFilter === 'thermal' && (<div className="absolute right-4 top-4 bottom-4 w-6 rounded-lg overflow-hidden">
